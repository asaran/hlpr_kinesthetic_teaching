...
for i in range(len(users)):
    ...
    for seg in d:
        ...
        if(demo_type=='k'):
            ...
            timeline, keyframes, open_keyframe, saccade_indices = get_color_timeline_with_seg(data, video_file, bag_file)
            ...
            if(demo_type=='k' and cond=='p'):
                plt.figure(1)
                plt.scatter(scaled_timeline,np.repeat(i,len(scaled_timeline)),color=timeline, s=5)

                # Mark keyframe boundaries
                for xc in keyframes:
                    plt.vlines(x=xc*10, color='red', linestyle='--', ymin=i-0.2, ymax=i+0.2)

                for xc in open_keyframe:
                    plt.vlines(x=xc*10, color='green', linestyle='--', ymin=i-0.2, ymax=i+0.2)
...
plt.figure(1)
title = 'Naive users KT demo: plate target'
plt.title(title)    
plt.savefig('vis/'+title)

plt.figure(2)
title = 'Naive users KT demo: bowl target'
plt.title(title)    
plt.savefig('vis/'+title)